...
# POSITION UPDATING
    def movement_update(self, bodies):
        ...
        # new position
        self.x += self.vx * dt + .5 * self.ax * dt * dt
        self.y += self.vy * dt + .5 * self.ay * dt * dt

        # new accel from new position
        fx, fy = self.force_calc(bodies)
        self.ax = fx/self.m
        self.ay = fy/self.m 

        # new velocity using avg of accels
        self.vx += .5 * (ax_old + self.ax) * dt
        self.vy += .5 * (ay_old + self.ay) * dt
...
for step in range(steps):
    ...
    # PLOTTING LOOP
    if step % 1000 == 0:  # plot every n steps because plotting is slower than computing
